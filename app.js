...
function initModals() {
  ...
  document.getElementById('logRecoveryBtn').addEventListener('click', () => {
    ...
    if (rec) { document.getElementById('recFitbit').value = rec.fitbit || ''; }
  ...
  document.getElementById('saveRecoveryBtn').addEventListener('click', () => {
    ...
    const entry = { date: today(), fitbit: fitbit||null };
...
async function fetchWeather() {
  try {
    const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=52.4538&longitude=-0.6268&current=temperature_2m,weathercode,windspeed_10m,precipitation&timezone=Europe%2FLondon');
